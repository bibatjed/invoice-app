...
export default function initializeInvoiceRouter(repository: IRepository) {
  ...
  router.route("/:invoice_tag").get(authMiddleware(repository), invoiceController.getDetailedInvoice).put(authMiddleware(repository), invoiceController.putInvoiceDetails).delete(authMiddleware(repository), invoiceController.deleteInvoice);
